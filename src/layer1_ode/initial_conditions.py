...
def _fetch_state_vector(body_id: int, epoch_jd: float) -> np.ndarray:
    ...
    en una época dada desde JPL Horizons.
    """
    # FIX: Se eliminó out_type="NO" (deprecated en astroquery >= 0.4.7)
    obj = Horizons(
        id=str(body_id),
        location="@10",          # Centro heliocéntrico (Sol)
        epochs=epoch_jd,
    )
    vec = obj.vectors(refplane="ecliptic")

    # FIX: Eliminados espacios finales en las claves para evitar KeyError
    return np.array([
        float(vec["x"][0]),
        float(vec["y"][0]),
        float(vec["z"][0]),
...
def get_initial_conditions(
    ...
    # Convertir época a JD si viene como string
    ...
    for name in perturbers:
        if name not in JPL_IDS:
            raise ValueError(f"Cuerpo '{name}' no reconocido. Opciones: {list(JPL_IDS.keys())}")
