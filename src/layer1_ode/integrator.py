"""
integrator.py
Núcleo RK45 N-cuerpos + Yarkovsky. Vectorizado y estable para producción.
"""
import numpy as np
...
def _build_rhs(order: list[str], gm_map: dict, A2: float):
    """RHS vectorizado y numéricamente estable."""
    n = len(order)
...
def _run_solver(rhs, y0, t_span, t_eval, dense_output):
    return solve_ivp(
        ...
        rtol=constants.RTOL, atol=constants.ATOL,
        dense_output=dense_output, max_step=constants.MAX_STEP_DAYS,
    )

def propagate(asteroid_id, epoch_start, t_years, dadt_au_my=0.0, 
              a_au=1.0, ecc=0.2, perturbers=None, dense_output=True):
    if perturbers is None:
        perturbers = constants.DEFAULT_PERTURBERS
    ...
    n_steps = max(int(abs(t_years) * 365.25), 100)  # Muestreo diario
